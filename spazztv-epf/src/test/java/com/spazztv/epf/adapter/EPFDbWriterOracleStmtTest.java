...
public class EPFDbWriterOracleStmtTest {
	...
	public void testDropTableStmt() {
		String expectedDropTableStmt = String.format(
				EPFDbWriterOracleStmt.DROP_TABLE_STMT, tableName);
	...
	public void testCreateTableStmt() {
		...
		while (entrySet.hasNext()) {
			...
			columnsToCreate += translateColumnType(colNType.getValue());
		...
		String expectedCreateTableStmt = String.format(
				EPFDbWriterOracleStmt.CREATE_TABLE_STMT, tableName,
				columnsToCreate);
	...
	public void testSetPrimaryKeyStmt() {
		...
		String expectedPrimaryKeyStmt = String.format(
				EPFDbWriterOracleStmt.PRIMARY_KEY_STMT, tableName, tableName + "_pk",
				expectedPrimaryKeyColumns);
	...
	public void testMergeTableStmt() {
		...
		String mergeWhere = String.format(
				EPFDbWriterOracleStmt.UNION_CREATE_WHERE, tableName,
				incTableName);
		...
		while (keyColumns.hasNext()) {
			...
			mergeWhere += " "
					+ String.format(EPFDbWriterOracleStmt.UNION_CREATE_JOIN,
							tableName, keyColumn, incTableName, keyColumn);
		}

		String execptedMergeTableStmt = String.format(
				EPFDbWriterOracleStmt.UNION_CREATE_PT1, unionTableName,
				incTableName);
		execptedMergeTableStmt += " "
				+ String.format(EPFDbWriterOracleStmt.UNION_CREATE_PT2,
	...
	public void testRenameTableStmt() {
		...
		String expectedRenameTableStmt = String.format(
				EPFDbWriterOracleStmt.RENAME_TABLE_STMT, unionTableName,
				tableName);
	...
	public String getInsertValues(int rowNumber) {
		...
		for (int i = 0; i < rowData.length; i++) {
			if (EPFDbWriterOracleStmt.UNQUOTED_TYPES.contains(columnTypes[i])) {
				values.append(rowData[i]);
			} else if (EPFDbWriterOracleStmt.DATE_TYPES.contains(columnTypes[i])) {
				values.append("to_date('" + rowData[i] + "','YYYY-MM-DD HH24:MI:SS')");
			} else {
				values.append("'" + rowData[i] + "'");
			}
	...
	private String translateColumnType(String columnType) {
		for (String typePat : EPFDbWriterOracleStmt.TRANSLATION_MAP.keySet()) {
			if (columnType.matches(typePat)) {
				return columnType.replaceAll(typePat,
						EPFDbWriterOracleStmt.TRANSLATION_MAP.get(typePat));
			}
		}
		return columnType;
	}
